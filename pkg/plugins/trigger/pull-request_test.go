...
func TestOrgInvitationGuidance(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			cfg:              plugins.OrgInviteConfig{Disabled: true},
			issues:           nil,
		...
		},
		{
			...
			cfg:  plugins.OrgInviteConfig{Message: "Please consider [joining us]({join_org_url}) for full access!"},
			issues: map[int]*github.Issue{
				1: mergedPRIssue(1, "author"),
				2: mergedPRIssue(2, "author"),
				3: mergedPRIssue(3, "author"),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			var c Client
			var capturingClient *findIssuesWithOrgCapturingClient

			if tc.expectNoSearch {
				capturingClient = &findIssuesWithOrgCapturingClient{
					FakeClient: fakegithub.NewFakeClient(),
				}
				c = Client{
					GitHubClient: capturingClient,
			...
			if tc.expectNoSearch && capturingClient.query != "" {
				t.Fatalf("expected no search call when disabled, got query %q", capturingClient.query)
			}
		})
	}
}

func TestEffectiveMergedPRThreshold(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		...
		{
			name:     "nil threshold returns default",
			cfg:      plugins.OrgInviteConfig{},
			expected: 3,
		},
		{
			name:     "explicit threshold of 5",
			cfg:      plugins.OrgInviteConfig{MergedPRThreshold: ptr.To(5)},
			expected: 5,
		},
		{
			name:     "explicit threshold of 0",
			cfg:      plugins.OrgInviteConfig{MergedPRThreshold: ptr.To(0)},
			expected: 0,
		},
		{
			name:     "explicit threshold of 1",
			cfg:      plugins.OrgInviteConfig{MergedPRThreshold: ptr.To(1)},
			expected: 1,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			result := effectiveMergedPRThreshold(tc.cfg)
			if result != tc.expected {
				t.Fatalf("effectiveMergedPRThreshold() = %d, want %d", result, tc.expected)
			}
		})
	}
}
